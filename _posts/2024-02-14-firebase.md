...
worrying about schemas. Plus, if your app runs without a server,
...
that, it consists in appending `[]` after a field name.
...
them. There's more! SKDB's reactive nature lets you create any
...
For example, imagine you want to be able to search products based on
their category, and you would like the result to be sorted by rating.
...
```sql
SELECT * FROM categroy_by_rating WHERE ...
```
...
SKDB. Also the other way around, you can watch for the local
